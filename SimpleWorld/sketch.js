...
function SimpleTree() {
  ...
  this.draw = () => {
    ...
    const TempPosition = {x: this.position.x, y: this.position.y, w: this.BranchSize.w},
  ...
  this.basicSetUp = ( Ground, z ) => {
    ...
    if( z === 'mid' ) {
      ...
      this.color = '#303030';
      this.TrunkSize = {w: random( 5, 15 ),
        h: random( 10, 20 )};
    } else {

      this.TrunkSize = {w: random( 15, 25 ),
        h: random( 20, 30 )};
    }

    this.BranchSize = {w: this.TrunkSize.w + random( 5, 15 ),
...
function SimpleHouse() {
  ...
  const drawDoor = () => {
    ...
    const DoorPosition = {x: ( this.position.x + ( this.BaseSize.w / 2 ) ) - ( this.Door.w / 2 ),
  ...
  this.basicSetUp = ( Ground, z ) => {
    ...
    this.RoofSize = {w: this.BaseSize.w * .25,
      h:  this.BaseSize.h * .4};

    this.Door = {w: this.BaseSize.w * .25,
      h: this.BaseSize.h * .45,
      NobRadius: 3};

    this.windows = {size: this.BaseSize.w * .25};
...
function SimpleMountain() {
  ...
  this.basicSetUp = ( Ground ) => {
    ...
    const SpaceBetween = random( LastPlacedThing.width * .6,  LastPlacedThing.width + 100 );
...
function Cloud() {
  ...
  this.basicSetUp = ( Ground, z ) => {
    ...
    for ( let i = 0; i <= this.numOfPoofs; i++ ) {
      ...
      const poof = {xOffSet: z === 'for' ? random( -65, 65 ) : random( -55, 55 ),
  ...
  this.activateWeather = () => {
    ...
    if( Enviorment.weather === 1 ) {
      ...
      // make rain
      const drop = {xOffSet: random( -50, 50 ),
      ...
      // make snow
      const	RandomNum = Number( random( 0,1 ).toFixed() ),
        flake = {
          xOffSet: random( -50, 50 ),
          yOffSet: random( 20, 30 ),
          size: this.wSize,
          counter: 0,
          floating: RandomNum === 1
        };
...
function Env() {
  ...
  this.GenerateClouds = () => {
    ...
    if( this.CloudTimer <= 0 && this.CloudsInView.length < 10 ) {
      ...
      const RanNum = Number( random( 0,1 ).toFixed() ),
        NewCloud = new Cloud;
...
const PickStartingPosition = ( width, least, most ) => {
  ...
  const SpaceBetween = random( least,  most );
...
const MakeClouds = ( least, most, ground, z ) => {
  ...
  const NumOfClouds = random( least, most ),
    SomeClouds = [];
...
const MakeLandThings = ( ground, z ) => {
  ...
  while ( LastPlacedThing.x < ground.size.w - 200 ) {
    const RandomNum = Number( random( 0,2 ).toFixed( 0 ) );
    const NewThing = RandomLandCreation( RandomNum );
...
const MakeMountains = ( ground, z ) => {
  ...
  while ( LastPlacedThing.x < ground.size.w - 200 ) {
    const NewMountain = new SimpleMountain;
...
const MakeForGround = () => {
  ...
  const Clouds = MakeClouds( 1, 3, TheGround, 'for' ),
...
const MakeMidGround = () => {
  ...
  const Clouds = MakeClouds( 1, 2, TheMidGround, 'mid' ),
...
const MakeBackGround = () => {
  ...
  const Mountains = MakeMountains( TheBackGround, 'back' );
...
const DrawAll = () => {
  ...
  const AllThings = BackGround.concat( MidGround ).concat( ForGround );
...
const AddAndRemoveClouds = () => {
  ...
  if( key === 'd' ) {
    ...
    const ranNum = Number( random( 0,Clouds.length - 1 ).toFixed() ), copiedCloud = Clouds[ranNum], newCloud = new Cloud;
